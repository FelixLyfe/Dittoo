...
struct ClipboardScreen: PaletteScreen {
    ...
    private var page: ClipboardStore.SearchPage {
        store.searchPage(vm.query, filter: vm.clipboardFilter, limit: vm.clipboardResultLimit)
    }

    var rows: [ClipboardItem] { page.items }
    var hasMore: Bool { page.hasMore }

    func loadMore() {
        if hasMore { vm.clipboardResultLimit += ClipboardStore.pageSize }
    }
        ...
        AnyView(
            content(selection: selection, scroll: scroll)
                .safeAreaInset(edge: .top, spacing: 0) {
                    ClipboardStorageStatus(store: store)
                }
...
/// Actions menu for an entry, shown bottom-right on right-click.
@MainActor
